...
- **Task 2 (indexing progress instrumentation)**: done. `sleuth/api/progress_store.py` (new, in-process thread-safe dict keyed by `repo_id`) + optional `on_event`/`on_batch_done` callbacks threaded through `ingest_repo`/`embed_batch` (default `None`, backward compatible). `GET /repos/{id}/progress` reads it back. `tests/test_embed.py`, `tests/test_pipeline.py`, `tests/test_api_repos.py` cover it. Also fixed a real environment bug: `TEST_DATABASE_URL` was byte-identical to prod `DATABASE_URL` (both pointed at Supabase) — repointed to local Docker Postgres (`localhost:5433`), full suite went from 163s to ~45s.
- **Task 3 (chat persistence schema + chat CRUD endpoints)**: done. `chats`/`messages` tables added to `schema.sql` (cascade delete from `repos`/`chats`). `sleuth/store.py` — `create_chat`/`list_chats`/`get_chat`/`create_message`/`list_messages`. `sleuth/api/routes/chat.py` (new) — `POST /chats` (404 unknown repo, 409 if repo not `ready`), `GET /chats?repo_id=`, `GET /chats/{id}/messages` (404 unknown chat) — router-level `Depends(require_session)`, same pattern as Task 1's `repos.router`. `tests/test_api_chat.py`. Plan doc's Task 3 sample test predates the real email/password auth flow (no session at all) — written against the actual established convention instead (signup via `TestClient`, session cookie), and added the auth dependency the draft omitted. `create_message`/`list_messages` have no caller yet — Task 4 (SSE streaming) is the first one.
...
- **Tasks 6-12** (login/landing/repos/indexing/chat screens, theme switcher, polish): not started yet.
